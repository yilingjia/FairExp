...
class SimulationArgumentParser(argparse.ArgumentParser):
    def __init__(self, description=None, set_arguments={}):
        ...
        self._sim_add_argument("--n_runs", dest="n_runs", default=1, type=int,
        ...
        self._sim_add_argument("--skip_store_bin_data", dest="store_binarized_data_after_read", action="store_false")
        ...
        self._sim_add_argument("--algo", dest="algo", default="PairRank", type=str, help="the learning algorithm", )

        self._sim_add_argument("--update", dest="update", default="gd", type=str,
                               help="how to update the model: gd, sgd, batch_sgd, online_sgd", )

        self._sim_add_argument("--learning_rate", dest="lr", default=0.1, type=float,
                               help="learning rate of optimization", )
        self._sim_add_argument("--learning_rate_decay", dest="lr_decay", default=0.9999977, type=float,
                               help="learning rate decay for optimization", )

        self._sim_add_argument("--refine", dest="refine", action="store_true",
                               help="refine the graph based on triangle inequality for PairRank and PairDiff", )
        self._sim_add_argument("--lambda", dest="lmbd", default=0.5, type=float, help="regularization parameter", )
        self._sim_add_argument("--alpha", dest="alpha", default=0.01, type=float, help="parameter for CB calculation", )
        self._sim_add_argument("--seed", dest="seed", default=0, type=int, help="random seed for different runs", )
        self._sim_add_argument("--rank", dest="rank", default="random", type=str,
                               help="how to rank the items in each partition in PairRank and PairDiff", )
        self._sim_add_argument("--ind", dest="ind", action="store_true",
                               help="whether to use independent pairs in PairRank")
        self._sim_add_argument("--epsilon", dest="epsilon", default=0.1, type=float, help="epsilon greedy")

        # FairExp arguments
        self._sim_add_argument("--fair_beta", dest="fair_beta", default=1.0, type=float, help="fairness control")
        self._sim_add_argument("--fair_epsilon", dest="fair_epsilon", default=0.1, type=float,
                               help="fairness threshold")
        self._sim_add_argument("--decay_mode", dest="decay_mode", default="overk", type=str,
                               help="decay mode for position bias.")
        self._sim_add_argument("--group", dest="group", default="inlink", type=str, help="group attribute for fairness")
        self._sim_add_argument("--unfairness", dest="unfairness", default="projected", type=str)
        self._sim_add_argument("--benign", dest="benign", action="store_true")
    ...
    def parse_sim_args(self):
        ...
        sim_args = {"description": self._description, }
    ...
    def parse_other_args(self, ranker_args=None, ranker=None):
        ...
        other_args = {}
        if ranker:
            other_args.update(ranker.default_ranker_parameters())
        for name, value in args.items():
    ...
    def parse_all_args(self, ranker_args=None, ranker=None):
        return (self.parse_sim_args(), self.parse_other_args(ranker_args=ranker_args, ranker=ranker))
