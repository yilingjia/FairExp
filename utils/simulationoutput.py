...
class FileOutput(object):
    def __init__(
        self, output_file_path, output_header=None, close_between_writes=False, also_print=False, write_date=False
    ):
...
def get_simulation_report(simulation_arguments):
    ...
    if not simulation_arguments.log_folder is None and os.path.isdir(simulation_arguments.log_folder):
        ...
        header = ["Starting simulation.", "WARNING: No log folder found, log is not stored elsewhere."]
...
class SimulationOutput(object):
    ...
    def __init__(
        self, simulation_arguments, simulation_name, dataset, num_click_models, ranker_arguments, output_averager
    ):
        ...
        self._closed = False
        self.output_path = "%s/%s/%s.out" % (self.output_folder, self.dataset_name, self.simulation_name)
        combined_args = {
            "simulation_arguments": vars(simulation_arguments),
            "ranker_arguments": ranker_arguments,
        }
        if self.print_output:
            output_header = json.dumps(combined_args, sort_keys=True, indent=4, separators=(",", ": "))
    ...
    def write_run_output(self, run_output):
        assert not self._closed, "Simulation Output (%s) written to after being closed." % self.output_path
...
class BufferPrintOutput(object):
    ...
    def pretty_run_write(self, run_index, run_output):
        ...
        run_lines = [
            ...
            "RUN TIME: %s (%.02f seconds)" % (timedelta(seconds=run_details["runtime"]), run_details["runtime"]),
        ]
