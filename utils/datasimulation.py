  ...
  """
    ...
    def run(self, ranker_tuples):
        ...
        for dataset in datasets:
            ...
            if not dataset.click_model_type in self.click_models:
                ...
                missing_models = get_click_models(self.sim_args.click_models + [dataset.click_model_type])
        ...
        print("Time taken: %s (%d seconds)" % (str(datetime.timedelta(seconds=seconds_past)), seconds_past))

        assert all(
            output.finished() for output in self.run_outputs.values()
    ...
    def load_datafold_processes(self, datafold, ranker_tuples):
        while self.folds_in_mem >= datafold.max_folds:
            self.wait_for_output()
            self.update_active()
        print("Read   %d: Fold %d of dataset %s." % (self.read_index, datafold.fold_num + 1, datafold.name))
        datafold.read_data()
        self.read_index += 1
        self.wait_for_output()
        self.update_active()
        for run_name, r_class, r_new_args in ranker_tuples:
            ...
            if not output_key in self.run_outputs:
                self.run_outputs[output_key] = SimulationOutput(
                    ...
                    run_name,
                    datafold,
                    len(self.click_models[datafold.click_model_type]),
                    r_args,
            ...
            for c_m in self.click_models[datafold.click_model_type]:
                ...
                for i in range(datafold.num_runs_per_fold):
                    new_proc = Process(target=self.start_run, args=(sim, output_key, ranker_setup, self.run_index))
                    self.processes.append((new_proc, datafold))
                    print(
                        "Launch %d: %s %d with click model %s on fold %d from dataset %s."
                        % (self.run_index, run_name, i, c_m.name, datafold.fold_num + 1, datafold.name,)
                    )
                    self.run_index += 1
    ...
    """
        ...
        for datafold in dead_datafolds:
            print("Clean  %d: Fold %d of dataset %s." % (self.clean_index, datafold.fold_num + 1, datafold.name))
            datafold.clean_data()
            self.clean_index += 1
    ...
    """
        ...
        try:
            while True:
                ...
                print(
                    "Output %d: %s on dataset %s. (%d/%d)"
                    % (
                        self._outputs_found,
                        output_key[0],
                        output_key[1],
                        sim_output.run_index + 1,
                        sim_output.expected_runs(),
                    )
                )
