...
class OutputAverager(object):
    ...
    def average_results(self, output_path):
        with open(output_path, "r") as f:
            ...
            cur_click_model = self.click_model_name(run_details["click model"])
            runtimes = {
            ...
            all_val = {}
            for name in all_ind:
                all_ind[name] = np.array(all_ind[name], dtype=np.int32)
                all_val[name] = {cur_click_model: [np.array(first_val[name], dtype=float)]}
            ...
            for line in f:
                ...
                runtimes[cur_click_model].append(float(run_details["runtime"]))
        ...
        for name, cur_ind in all_ind.items():
            cur_results = {"indices": cur_ind.tolist()}
        ...
        output = {"simulation_arguments": sim_args, "runtimes": average_runtimes, "results": results}
    ...
    def create_average_file(self, sim_output):
        ...
        with open(self.output_path, "w") as w:
            ...
            print(
                "Closed %d: %s on %s was averaged and stored."
                % (self._average_index, sim_output.simulation_name, sim_output.dataset_name)
            )
