...
class BasicOnlineRanker(object):
    ...
    def get_test_rankings(self, features, query_ranges, inverted=True):
        return rnk.rank_multiple_queries(np.zeros(features.shape[0]), query_ranges, inverted=inverted,
                                         n_results=self.n_results)

    def get_query_features(self, query_id, features, query_ranges):
        start_i = query_ranges[query_id]
        end_i = query_ranges[query_id + 1]
        return features[start_i:end_i, :]

    def get_query_label(self, query_id, label_vector, query_ranges):
        start_i = query_ranges[query_id]
        end_i = query_ranges[query_id + 1]
    ...
    def get_train_query_ranking(self, query_id, inverted=False):
        ...
        query_feat = self.get_query_features(query_id, self._train_features, self._train_query_ranges)
        self._last_ranking = self._create_train_ranking(query_id, query_feat, inverted)[:self.n_results]
        return self._last_ranking

    def _create_train_ranking(self, query_id, query_feat, inverted):
        n_docs = self.get_query_size(query_id, self._train_query_ranges)
        return rnk.rank_single_query(np.zeros(n_docs), inverted=inverted, n_results=self.n_results)[:self.n_results]
